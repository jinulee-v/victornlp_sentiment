...
if 'sentiment' in input.keys():
  # Golden sentiment label is given
...
from .model.model import sentiment_model
from .model.loss import sentiment_loss_fn
from .model.run import sentiment_run_fn
from .tools.analyze import sentiment_analysis_fn
...
def parse_cmd_arguments():
  ...
  parser.add_argument('-a', '--analyze', type=str, action='append', choices=sentiment_analysis_fn)
...
def main():
  ...
  with open(labels_path) as type_label_file:
    type_label = json.load(type_label_file)['sentiment_labels']
  ...
  model = sentiment_model[model_name](embedding_objs, type_label, model_config)
  model.load_state_dict(torch.load(args.model_dir + '/model.pt'))
  model = model.to(device)

  run_fn = sentiment_run_fn[train_config['run_fn']]
  ...
  with torch.no_grad():
    if from_file:
      ...
      if not args.analyze:
        return
      analyzers = {name:sentiment_analysis_fn[name] for name in args.analyze}
